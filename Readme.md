...
[![PyPI](https://img.shields.io/pypi/v/tinify?color=brightgreen)](https://pypi.org/project/tinify/)
[![Downloads](https://pepy.tech/badge/tinify)](https://pypi.org/project/tinify/#files)
...
```bash
git clone https://github.com/InterDigitalInc/CompressAI tinify
cd tinify
...
```bash
python3 -m tinify.utils.eval_model checkpoint /path/to/images/folder/ -a $ARCH -p $MODEL_CHECKPOINT...
```

To evaluate provided pre-trained models:

```bash
python3 -m tinify.utils.eval_model pretrained /path/to/images/folder/ -a $ARCH -q $QUALITY_LEVELS...
```

To plot results from bench/eval_model simulations (requires matplotlib by default):

```bash
python3 -m tinify.utils.plot --help
```

To evaluate traditional codecs:

```bash
python3 -m tinify.utils.bench --help
python3 -m tinify.utils.bench bpg --help
python3 -m tinify.utils.bench vtm --help
```

For video, similar tests can be run, CompressAI only includes ssf2020 for now:

```bash
python3 -m tinify.utils.video.eval_model checkpoint /path/to/video/folder/ -a ssf2020 -p $MODEL_CHECKPOINT...
python3 -m tinify.utils.video.eval_model pretrained /path/to/video/folder/ -a ssf2020 -q $QUALITY_LEVELS...
python3 -m tinify.utils.video.bench x265 --help
python3 -m tinify.utils.video.bench VTM --help
python3 -m tinify.utils.video.plot --help
```
...
```bash
pytest -sx --cov=tinify --cov-append --cov-report term-missing tests
```
