...
# copy src
if [ ! -d src/tinify/sadl_codec ]; then
 mkdir -p src/tinify;
 cd src;
 cp -R ${BDIR}/../sadl_codec tinify/;
 ln -s ${BDIR}/../../sadl .;
...
cd build/sadl_codec;
cmake --log-level=ERROR ../../src/tinify/sadl_codec >$MESSAGE;
...
cp decoder_float.h model_cdfs.h src/tinify/sadl_codec/
mkdir -p build/sadl_codec
cd build/sadl_codec;
...
if (( WITH_QUANT )); then 
 ...
 cp decoder_int16.h src/tinify/sadl_codec;
 mkdir -p build/sadl_codec
 cd build/sadl_codec;
...
cp encoder_float.h src/tinify/sadl_codec/
mkdir -p build/sadl_codec
cd build/sadl_codec;
# cmake --log-level=ERROR ../../src/tinify/sadl_codec >$MESSAGE;
...
if (( WITH_QUANT )); then 
    ...
    cp encoder_int16.h src/tinify/sadl_codec/
    mkdir -p build/sadl_codec
    cd build/sadl_codec;
