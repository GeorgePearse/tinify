...
2- Conversion
 ...
 ```shell
 tinify/sadl_codec/build_codec.sh --model model.pth --training_dataset trainingdataset.npy
 ```
...
```shell
tinify/sadl_codec/check_kodak.sh --dir kodak_dir
```
or for better performance (longer to generate):
```shell
tinify/sadl_codec/check_kodak.sh --dir kodak_dir --rdoq lambda
```
